...
def mk_simu((i, tree_filename, OutDirNamePrefixTree), n_try = 0) :
    ...
    for (c1,c2) in SampledCATcouples:
        k_couple+=1
        logger.info("Scenario %s/%s - Couple %s/%s, C_anc: %s, C_conv: %s",i, Nbsimul, k_couple, Nb_sampled_couple, sim_profiles.get_profile_name(c1), sim_profiles.get_profile_name(c2))
