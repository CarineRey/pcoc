...
def make_estim_conv_topo(name, c1, g_tree, est_profiles, suffix="", gamma = False, max_gap_allowed=90):
    ...
    command="bppml param=%s NAME=%s SUFFIX=%s REP_SEQ=%s TREE=%s REP_EST=%s \'FILESEQ=%s\' " \
        %(repbppconfig + "/CATseq_conv.bpp", name, suffix, repseq, tree_fn, repest, fasta_file)
    number_of_models = 0
...
>>>>>>> 3f39b19... start model mixture (not yet working)
